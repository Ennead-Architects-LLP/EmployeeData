import logging
from typing import Optional

from .config import ScraperConfig
from .complete_scraper import CompleteScraper

class Orchestrator:
    def __init__(self, config: Optional[ScraperConfig] = None):
        self.config = config or ScraperConfig.from_env()
        self.logger = logging.getLogger(__name__)

    async def run(self) -> str:
        self.logger.info("Starting orchestrator")
        self.config.setup_directories()

        scraper = CompleteScraper(self.config)

        # Step 1: Scrape employees
        employees = await scraper.scrape_all_employees()
        if not employees:
            self.logger.error("No employees scraped. Aborting.")
            return ""

        # Step 2: Save JSON
        json_path = scraper.save_to_json(employees)
        self.logger.info(f"Saved JSON to {json_path}")

        # Step 3: Generate HTML
        html_path = scraper.generate_html_report(json_path)
        self.logger.info(f"Generated HTML at {html_path}")

        return html_path
